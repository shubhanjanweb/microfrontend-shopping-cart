...
export class ProductService {
  ...
  async createProduct(createProductDto: ProductDto): Promise<string> {
    ...
    const product: ProductEntity | void = await this.productRepository.save(newProduct).catch(err => {
      throw new HttpException(`${productName} - ${err}`, HttpStatus.BAD_REQUEST);
    });
  ...
  //Read
  async productList(req: any): Promise<ProductVo[]> {
